...
export class PlanSynthesis {
    static synthesize(cm1, cm2, cm3) {
        const { term, wellness_direction, vulnerability_points: baseVulnerabilities, } = cm1;
        const { seasonal_bias, diet_adjustment: regionalDietNote } = cm2;
        const { vulnerability_points: constitutionVulnerabilities, direction_override, intensity_modifier, focus_shift, diet_additions, diet_restrictions, } = cm3;
    ...
    static calculateIntensity(intensity_modifier, seasonal_bias) {
        let factor = 1.0;
    ...
    static buildHealthPlan({ term, wellness_direction, direction_override, baseVulnerabilities, constitutionVulnerabilities, intensityFactor, seasonal_bias, }) {
    ...
    static generateHighRisk({ term, vulnerabilities, intensity }) {
        const riskMap = {
            '暑热': ['中暑', '脱水', '热射病'],
            '湿热': ['湿疹', '腹泻', '暑湿感冒'],
            '秋燥': ['皮肤干燥', '咳嗽', '便秘'],
    ...
    static generatePrevention({ term, direction, intensity, seasonal_bias }) {
        const suggestions = [];
        if (intensity >= 1.2)
            suggestions.push('注意防暑降温');
        if (seasonal_bias && seasonal_bias.temp_adjustment > 1.1)
            suggestions.push('避免正午外出');
        if (direction && direction.includes('温阳'))
            suggestions.push('注意保暖');
        if (direction && direction.includes('清热'))
            suggestions.push('保持充足水分');
        return suggestions;
    }
    static generateVulnerabilityDescription({ term, vulnerabilities, intensity }) {
        if (vulnerabilities.length === 0) {
            return '当前节气相对平稳，保持日常养生即可';
        }
        return `${term}期间，体质脆弱点：${vulnerabilities.join('；')}。建议重点关注。`;
    }
    static buildDailyRoutine({ term, wellness_direction, intensityFactor, seasonal_bias }) {
    ...
    static generateSleepAdvice({ term, intensity }) {
        const adviceMap = {
    ...
    static generateExerciseAdvice({ term, intensity, seasonal_bias }) {
        if (intensity >= 1.2)
            return '避免剧烈运动，推荐散步、太极、八段锦';
        if (term.includes('暑'))
            return '清晨或傍晚运动，推荐游泳、太极';
        if (term.includes('寒'))
            return '室内运动为主，推荐八段锦、太极拳';
        return '适度运动，推荐散步、太极、八段锦';
    }
    static generateEmotionAdvice({ term, direction }) {
        if (direction && direction.includes('疏肝'))
            return '保持心情舒畅，适当宣泄情绪';
        if (direction && direction.includes('养心'))
            return '静心养性，避免急躁';
        if (direction && direction.includes('温阳'))
            return '保持乐观心态，多晒太阳';
        return '保持平和心态，顺应节气规律';
    }
    static buildDietPlan({ term, wellness_direction, diet_additions, diet_restrictions, regionalDietNote, intensityFactor, }) {
        return {
    ...
    static buildHerbalTeaPlan({ term, direction_override, intensityFactor }) {
        return {
