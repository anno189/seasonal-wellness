...
export class WeatherCorrection {
    ...
    static normalizeWeather(weather) {
        ...
        if (weatherStr.includes('晴') || weatherStr.includes('sun'))
            return '晴天';
        if (weatherStr.includes('阴') || weatherStr.includes('overcast'))
            return '阴天';
        if (weatherStr.includes('雨') || weatherStr.includes('rain')) {
            if (weatherStr.includes('雷'))
                return '雷阵雨';
            if (weatherStr.includes('大'))
                return '大雨';
            if (weatherStr.includes('中'))
                return '中雨';
            return '小雨';
        }
        if (weatherStr.includes('高温') || weatherStr.includes('heat'))
            return '高温';
        if (weatherStr.includes('低温') || weatherStr.includes('cold'))
            return '低温';
        if (weatherStr.includes('雾') || weatherStr.includes('霾') || weatherStr.includes('haze'))
            return '雾霾';
        return '多云';
    }
    static adjustRecipeWeights(recipes, correction) {
        return recipes.map(recipe => {
            let weight = 1.0;
            if (recipe.category === '清热')
                weight *= correction.cool_food_weight;
            else if (recipe.category === '祛湿')
                weight *= correction.moist_food_weight;
            else if (recipe.category === '温补')
                weight *= correction.warm_food_weight;
            return { ...recipe, weather_weight: weight };
        });
    }
    static adjustTeaWeights(teas, correction) {
        return teas.map(tea => {
            let weight = 1.0;
            if (tea.direction.includes('清热') || tea.direction.includes('解暑'))
                weight *= correction.cool_food_weight;
            else if (tea.direction.includes('祛湿'))
                weight *= correction.moist_food_weight;
            else if (tea.direction.includes('温补') || tea.direction.includes('温阳'))
                weight *= correction.warm_food_weight;
