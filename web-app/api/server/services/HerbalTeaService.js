...
function classifyTea(ingredients) {
    let coolScore = 0, warmScore = 0, moistScore = 0;
    for (const ing of ingredients) {
        const name = ing.replace(/[\d\.]+g?$/g, '').trim();
        if (COOL_INGREDIENTS.some(k => name.includes(k)))
            coolScore += 2;
        if (WARM_INGREDIENTS.some(k => name.includes(k)))
            warmScore += 2;
        if (MOIST_INGREDIENTS.some(k => name.includes(k)))
            moistScore += 2;
    }
...
export class HerbalTeaService {
    static getTeaRecommendation(term, constitution, weather = null, date = null) {
        ...
        if (!pool) {
            ...
            if (fallbackPool)
                return this.getFromPool(fallbackPool, weather, date);
            return this.getDefaultTea(term);
        }
        return this.getFromPool(pool, weather, date);
    }
    static getFromPool(pool, weather, date) {
        ...
        if (teaPool.length === 0)
            return this.getDefaultTea(pool.solar_term);
        ...
        const scoredTeas = teaPool.map((tea) => {
    ...
    static generateWeatherNote(weather, direction, weights) {
        const notes = {
